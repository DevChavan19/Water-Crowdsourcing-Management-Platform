...
import com.watercrowdsourcing.ums_service.custom_exceptions.InvalidInputException;
import com.watercrowdsourcing.ums_service.custom_exceptions.ResourceNotFoundException;
...
import com.watercrowdsourcing.ums_service.entities.Role;
import com.watercrowdsourcing.ums_service.repository.UserRepository;
import com.watercrowdsourcing.ums_service.security.JwtUtils;
import com.watercrowdsourcing.ums_service.security.UserPrincipal;
...
public class UserServiceImpl implements UserService {
	...
	@Override
	public ApiResponse departmentSignup(com.watercrowdsourcing.ums_service.dtos.DepartmentSignupRequest request) {
		if (userRepository.existsByEmail(request.getEmail())) {
			throw new InvalidInputException("Department user email already exists");
		}

		User deptUser = new User();
		deptUser.setFirstName(request.getFirstName());
		deptUser.setLastName(request.getLastName());
		deptUser.setEmail(request.getEmail());
		deptUser.setPhone(request.getPhone());
		deptUser.setPassword(passwordEncoder.encode(request.getPassword()));
		deptUser.setRole(Role.DEPARTMENT);
		deptUser.setDepartmentId(request.getDepartmentId());
		deptUser.setVerified(true);
		deptUser.setDeleted(false);

		userRepository.save(deptUser);
		return new ApiResponse("SUCCESS", "Department user registered successfully");
	}
